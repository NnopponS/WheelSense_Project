...
> **Version**: 3.3.0 | **Last audit**: 2026-04-01 | **Test coverage**: 79% (47 passed)
>
...
### Motion Training & Prediction Flow (XGBoost)

```
POST /api/motion/train
    ├─ Query motion_training_data from DB (active workspace)
...
> Recording endpoints send MQTT commands to devices. ML endpoints train/predict using XGBoost.
...
| `POST` | `/api/motion/predict` | `MotionPredictRequest` | Predict action from IMU window |
| `GET` | `/api/motion/model` | — | Model info (status, labels, accuracy) |
| `POST` | `/api/motion/model/save` | — | Persist model to disk |
| `POST` | `/api/motion/model/load` | — | Load model from disk |
...
| `pytest` | 47 passed, 79% cov | Localization 100%, MQTT 82%, Feature Eng 100%, Motion Cls 100% |
...
6. **JSONB columns** — Always use `JSON().with_variant(JSONB, "postgresql")`, NEVER raw `JSONB`
7. **KNN model is in-memory** — NOT persisted across restarts, retrain via `/api/localization/retrain`
8. **XGBoost model can be saved** — Use `/api/motion/model/save` + `/api/motion/model/load` for persistence
9. **IMU rate = 20Hz** — Window of 40 samples = 2 seconds (see firmware Config.h)
...
- **XGBoost motion classification (train/predict/save/load)** ✨ NEW
- **Feature engineering pipeline (sliding window, ~35 features)** ✨ NEW
- Camera registration + command forwarding
- Security audit passing
- Test suite (79% coverage, 47 tests)
...
- KNN model persistence (save/load to disk — XGBoost already has this)
