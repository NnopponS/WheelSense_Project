...
async def reset_simulator_workspace(workspace_name: str | None = None) -> dict[str, Any]:
    ...
    """
    from scripts.seed_demo import run_sim_team_seed
    
    target_workspace = workspace_name or settings.bootstrap_demo_workspace_name or "WheelSense Demo Workspace"
    ...
    async with AsyncSessionLocal() as session:
        ...
        if workspace is None:
            # Workspace doesn't exist - just run full seed
            workspace_id = await run_sim_team_seed(target_workspace, reset=False)
            return {
                "action": "created",
                "workspace_id": workspace_id,
                "workspace_name": target_workspace,
                "message": "Created new simulator workspace with baseline data",
            }
        
        # Clear dynamic data
        cleared = await clear_workspace_dynamic_data(session, workspace.id)
        
        # Reset device assignments (deactivate all, will be re-created)
        await session.execute(
            delete(PatientDeviceAssignment).where(
                PatientDeviceAssignment.workspace_id == workspace.id
            )
        )
        await session.commit()
    ...
    return {
        ...
        "message": "Simulator workspace reset to baseline state",
    }
