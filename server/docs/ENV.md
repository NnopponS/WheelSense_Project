...
| `EASEAI_DETERMINISTIC_ANSWER_LOCK_ENABLED` | `true` (`false` in core compose) | When `false`, the deterministic answer locks are bypassed so the ADR 0015 LLM tool router runs as the **primary** decision-maker: (1) task-create clarification no longer locks a fixed question, (2) deterministic read synthesis for timeline/location/device/system/inventory/patient-navigation is skipped, and (3) the v2 `deterministic_immediate` read-only tool precheck is skipped. Only the **page-context** and **identity** fast paths remain, because the LLM router has no `page_context` in its system prompt. Layer 1 safety rejects (empty/overlong/role-disallowed) are unaffected. Set `true` to restore the legacy locked-answer behavior. |
