...
export function Drawer() {
    const { drawerOpen, drawerContent, closeDrawer, rooms, appliances, toggleAppliance, patients, wheelchairs } = useApp();
            ...
            <div className={`drawer ${drawerOpen ? 'open' : ''}`}>
                <div className="modal-header">
                    <span className="modal-title">
                        {type === 'wheelchair' && '🦽 รายละเอียด Wheelchair'}
                        {type === 'room' && '🏠 รายละเอียดห้อง'}
                        {type === 'patient' && '👤 รายละเอียดผู้ป่วย'}
                        {type === 'patient-edit' && '✏️ แก้ไขข้อมูลผู้ป่วย'}
                    </span>
                        ...
                        <div style={{ padding: '1.5rem' }}>
                            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                                ...
                                <span className={`list-item-badge ${data.status}`} style={{ marginTop: '0.5rem' }}>
                                    {data.status === 'normal' ? 'ปกติ' : data.status === 'warning' ? 'ระวัง' : data.status === 'alert' ? 'ฉุกเฉิน' : 'Offline'}
                                </span>
                            </div>

                            <div className="card" style={{ marginBottom: '1rem' }}>
                                <div className="card-header"><span className="card-title"><User size={16} /> ผู้ใช้งาน</span></div>
                                <div className="card-body">
                                    {data.patientName ? (
                                        ...
                                        <p style={{ color: 'var(--dark-text-muted)' }}>ไม่มีผู้ใช้งาน</p>
                                    )}
                                </div>
                            </div>

                            <div className="card" style={{ marginBottom: '1rem' }}>
                                <div className="card-header"><span className="card-title"><MapPin size={16} /> ตำแหน่งปัจจุบัน</span></div>
                                <div className="card-body">
                                    <div style={{ fontSize: '1.25rem', fontWeight: 600, color: 'var(--primary-400)' }}>
                                        {rooms.find(r => r.id === data.room)?.name || 'ไม่ทราบตำแหน่ง'}
                                    </div>
                                    <div style={{ fontSize: '0.8rem', color: 'var(--dark-text-muted)', marginTop: '0.25rem' }}>
                                        อยู่ที่นี่ตั้งแต่ {data.lastSeen ? new Date(data.lastSeen).toLocaleTimeString('th-TH') : '-'}
                                    </div>
                                </div>
                            </div>
                    ...
                    {type === 'room' && (
                        <div style={{ padding: '1.5rem' }}>
                            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                                ...
                                <h3>{data.name}</h3>
                                <p style={{ color: 'var(--dark-text-muted)' }}>{data.nameEn}</p>
                                <span className={`list-item-badge ${data.occupied ? 'normal' : 'offline'}`}>
                                    {data.occupied ? '🟢 มีคนอยู่' : '⚪ ว่าง'}
                                </span>
                            </div>
                                ...
                                <div className="video-stream" style={{ aspectRatio: '16/10', position: 'relative', background: 'linear-gradient(135deg, var(--dark-bg), var(--dark-surface))' }}>
                                    ...
                                    >
                                        ...
                                        <p style={{ margin: 0, textAlign: 'center' }}>
                                            ...
                                            {!streamMode && 'รอวิดีโอจากกล้อง'}
                                        </p>
                                        <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.75rem', opacity: 0.7, textAlign: 'center' }}>
                                            {streamMode === 'loading' && 'กำลังเชื่อมต่อกับ ESP32...'}
                                            {streamMode === 'offline' && 'ไม่สามารถเชื่อมต่อได้'}
                                            {streamMode === 'websocket' && 'เชื่อมต่อสำเร็จ'}
                                        </p>
                                    </div>
                                    {streamMode === 'websocket' && (
                            ...
                            <div className="card">
                                <div className="card-header">
                                    <span className="card-title"><Power size={16} /> ควบคุมเครื่องใช้ไฟฟ้า</span>
                                </div>
                                <div className="card-body">
                                    ...
                                    {(() => {
                                        ...
                                        return roomAppliances.length === 0 ? (
                                            <p style={{ color: 'var(--dark-text-muted)', textAlign: 'center' }}>ไม่มีเครื่องใช้ไฟฟ้าในห้องนี้</p>
                                        ) : null;
                                    })()}
                                </div>
                            </div>
                ...
                {(type === 'patient-edit' || type === 'wheelchair-edit') && (
                    <div className="modal-footer">
                        <button className="btn btn-secondary" onClick={closeDrawer}>ยกเลิก</button>
                        <button className="btn btn-primary" onClick={closeDrawer}>บันทึก</button>
                    </div>
                )}
            </div>
        </>
