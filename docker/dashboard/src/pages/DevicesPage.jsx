...
function VideoStreamPlayer({ room, onClose }) {
    ...
    return (
        <div className="card">
            <div className="card-header">
                <span className="card-title"><Video size={16} /> {room.name}</span>
                <span style={{
                    fontSize: '0.75rem',
                    ...
                    {streamMode === 'loading' && '⏳ กำลังเชื่อมต่อ...'}
                    {streamMode === 'offline' && '○ ไม่มีสัญญาณ'}
                </span>
            </div>
            <div className="video-stream" style={{ aspectRatio: '16/10', position: 'relative', background: 'linear-gradient(135deg, var(--bg-secondary), var(--bg-tertiary))' }}>
                ...
                <div style={{
                    ...
                    <p style={{ margin: 0, textAlign: 'center' }}>
                        ...
                        {streamMode === 'offline' && 'ไม่มีสัญญาณจากกล้อง'}
                    </p>
                </div>
                {streamMode === 'websocket' && <span className="video-live-badge" style={{ zIndex: 2 }}>LIVE</span>}
            </div>
        </div>
    );
}
...
export function DevicesPage() {
    const { devices, setDevices, rooms, role } = useApp();
    ...
    return (
        <div className="page-content">
            <div className="page-header">
                <h2>🔌 Devices & Nodes</h2>
                <p>จัดการอุปกรณ์ Node และ Gateway ในระบบ</p>
            </div>
            ...
            {activeTab === 'nodes' && (
                <div className="card">
                    <div className="card-header">
                        <span className="card-title"><Cpu size={18} /> Nodes</span>
                        {role === 'admin' && (
                            <button className="btn btn-primary"><Plus size={16} /> เพิ่ม Node</button>
                        )}
                    </div>
            ...
            {activeTab === 'gateways' && (
                <div className="card">
                    <div className="card-header">
                        <span className="card-title"><Settings size={18} /> Gateways</span>
                    </div>
                    <div className="table-container">
                        <table className="table">
                            <thead>
                        ...
                        <div className="card-body">
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem' }}>
                                ...
                                >
                                    ...
                                    <div style={{ fontWeight: 500 }}>📺 ดูทุกห้อง</div>
                                </button>
                    ...
                    {selectedRoom && (
                        <div style={{
                            ...
                            {(selectedRoom === 'all' ? rooms : rooms.filter(r => r.id === selectedRoom)).map(room => (
                                <VideoStreamPlayer key={room.id} room={room} />
                            ))}
                        </div>
                    )}
                    ...
                    {!selectedRoom && (
                        <div className="card">
                            <div className="card-body" style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
                                ...
                                <p>กรุณาเลือกห้องที่ต้องการดูกล้อง หรือกด "ดูทุกห้อง" เพื่อดูพร้อมกัน</p>
                            </div>
                        </div>
                    )}
                </div>
