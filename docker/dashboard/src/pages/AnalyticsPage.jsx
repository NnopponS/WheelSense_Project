...
export function AnalyticsPage() {
    ...
    const roomUsage = [
        { room: 'ห้องนอน', percentage: 35, color: 'var(--primary-500)' },
        { room: 'ห้องนั่งเล่น', percentage: 30, color: 'var(--success-500)' },
        { room: 'ห้องครัว', percentage: 20, color: 'var(--warning-500)' },
        { room: 'ห้องน้ำ', percentage: 15, color: 'var(--info-500)' },
    ];
    ...
    return (
        <div className="page-content">
            <div className="page-header">
                ...
                <p>วิเคราะห์ข้อมูลและแนวโน้มของระบบ</p>
            </div>

            {/* Date Range Filter */}
            <div className="tabs" style={{ marginBottom: '1.5rem', maxWidth: 400 }}>
                <button className={`tab ${dateRange === 'today' ? 'active' : ''}`} onClick={() => setDateRange('today')}>วันนี้</button>
                <button className={`tab ${dateRange === 'week' ? 'active' : ''}`} onClick={() => setDateRange('week')}>7 วัน</button>
                <button className={`tab ${dateRange === 'month' ? 'active' : ''}`} onClick={() => setDateRange('month')}>30 วัน</button>
            </div>
            ...
            <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
                <div className="stat-card">
                    ...
                    <div className="stat-content">
                        ...
                        <p>ผู้ป่วยออนไลน์</p>
                        <div className="stat-trend up"><TrendingUp size={14} /> +2 จากเมื่อวาน</div>
                    </div>
                </div>
                <div className="stat-card">
                ...
                <div className="stat-card">
                    ...
                    <div className="stat-content">
                        <h3>{todayAlerts}</h3>
                        <p>Alerts วันนี้</p>
                        <div className="stat-trend down"><TrendingDown size={14} /> -3 จากเมื่อวาน</div>
                    </div>
                </div>
                <div className="stat-card">
            ...
            <div className="content-grid">
                ...
                <div className="card">
                    <div className="card-header">
                        <span className="card-title"><BarChart3 size={18} /> กิจกรรมตามเวลา</span>
                    </div>
                ...
                <div className="card">
                    <div className="card-header">
                        <span className="card-title"><Clock size={18} /> การใช้งานห้อง</span>
                    </div>
                    <div className="card-body">
            ...
            <div className="card" style={{ marginTop: '1.5rem' }}>
                <div className="card-header">
                    <span className="card-title"><Users size={18} /> สถานะสุขภาพผู้ป่วย</span>
                </div>
                            ...
                            {patients.map(patient => {
                                ...
                                return (
                                    <tr key={patient.id}>
                                        ...
                                        <td>
                                            <span className={`list-item-badge ${patient.condition === 'ปกติ' ? 'normal' : patient.condition === 'ต้องระวัง' ? 'warning' : 'alert'}`}>
                                                {patient.condition}
                                            </span>
                                        </td>
                                        <td>{wc?.name || '-'}</td>
            ...
            <div className="card" style={{ marginTop: '1.5rem' }}>
                <div className="card-header">
                    <span className="card-title"><Activity size={18} /> AI Insights</span>
                </div>
                <div className="card-body">
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
                        <div style={{ padding: '1rem', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-lg)' }}>
                            <h4 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                <CheckCircle size={18} color="var(--success-500)" /> รายงานประจำวัน
                            </h4>
                        ...
                        {aiAnalysis.anomalies.map((anomaly, i) => (
                            <div key={i} style={{ padding: '1rem', background: 'rgba(239, 68, 68, 0.1)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--danger-500)' }}>
                                <h4 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', color: 'var(--danger-500)' }}>
                                    <AlertTriangle size={18} /> ⚠️ พบความผิดปกติ
                                </h4>
            ...
            <div className="card" style={{ marginTop: '1.5rem' }}>
                <div className="card-header">
                    <span className="card-title"><Cpu size={18} /> สถานะระบบ</span>
                </div>
                <div className="card-body">
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem' }}>
                        <div>
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Devices Online</div>
                            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--success-500)' }}>{onlineDevices}/{devices.length}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Active Emergencies</div>
                            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: activeEmergencies > 0 ? 'var(--danger-500)' : 'var(--success-500)' }}>
                                {activeEmergencies}
                            </div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>AI Last Analysis</div>
                            <div style={{ fontSize: '1rem', fontWeight: 500 }}>{new Date(aiAnalysis.lastAnalysis).toLocaleTimeString('th-TH')}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Uptime</div>
