...
# Activity-to-action/location mapping based on common schedule patterns
ACTIVITY_DEFAULTS = {
    "Wake up": {
        "action": {
            "devices": [
...
class ActivityDerivationService:
    ...
    def add_activity_mapping(self, activity: str, action: dict = None, location: str = None) -> bool:
        ...
        if location:
            ...
            if location_lower not in ROOM_NAMES and location not in ROOM_NAMES.values():
                return False
        ...
        if action:
            ...
            for device_spec in action["devices"]:
                ...
                # Accept both lowercase IDs and English names for rooms
                room_normalized = room.lower().replace(" ", "") if room else ""
                if room_normalized not in ROOM_NAMES and room not in ROOM_NAMES.values():
                    return False
