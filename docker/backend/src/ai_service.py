...
class AIService:
    ...
    def _detect_anomalies(self, patterns: Dict, activities: List[Dict]) -> List[Dict]:
        ...
        if bathroom_time > 30:
            anomalies.append({
                ...
                "message": f"ผู้ใช้อยู่ในห้องน้ำนานผิดปกติ ({bathroom_time:.0f} นาที)"
            })
        ...
        if bedroom_time > 240:  # 4 hours
            anomalies.append({
                ...
                "severity": "low",
                "message": f"ผู้ใช้อยู่ในห้องนอนเป็นเวลานาน ({bedroom_time:.0f} นาที)"
            })
        ...
        if total_visits == 0:
            anomalies.append({
                "type": "no_activity",
                "severity": "high",
                "message": "ไม่พบการเคลื่อนไหวของผู้ใช้"
            })
        ...
        if night_activity > 5:
            anomalies.append({
                "type": "unusual_hours",
                "severity": "low",
                "message": f"พบการเคลื่อนไหวในช่วงกลางคืนบ่อยครั้ง ({night_activity} ครั้ง)"
            })
    ...
    async def _get_gemini_analysis(
        ...
        # Prepare prompt
        prompt = f"""คุณเป็นผู้เชี่ยวชาญด้านการดูแลผู้สูงอายุและผู้ใช้รถเข็น กรุณาวิเคราะห์พฤติกรรมต่อไปนี้:

รูปแบบการใช้งาน:
- เวลาในแต่ละห้อง: {json.dumps(patterns['room_time'], ensure_ascii=False)}
- จำนวนครั้งที่เข้าห้อง: {json.dumps(patterns['room_visits'], ensure_ascii=False)}
- การใช้เครื่องใช้ไฟฟ้า: {json.dumps(patterns['appliance_usage'], ensure_ascii=False)}

ความผิดปกติที่ตรวจพบ:
{json.dumps(anomalies, ensure_ascii=False, indent=2)}

กรุณา:
1. วิเคราะห์รูปแบบพฤติกรรมโดยรวม
2. ประเมินความเสี่ยงด้านสุขภาพหรือความปลอดภัย
3. ให้คำแนะนำสำหรับผู้ดูแล
4. เสนอแนะการปรับปรุงคุณภาพชีวิต
    ...
    async def generate_recommendations(self, analysis: Dict) -> List[Dict]:
        ...
        if livingroom_time < 60:  # Less than 1 hour
            recommendations.append({
                "type": "activity",
                "priority": "medium",
                "title": "แนะนำให้ใช้เวลาในห้องนั่งเล่นมากขึ้น",
                "description": "การใช้เวลาในพื้นที่ส่วนกลางช่วยส่งเสริมการมีปฏิสัมพันธ์ทางสังคม"
            })
        ...
        for anomaly in anomalies:
            if anomaly.get("type") == "prolonged_stay" and anomaly.get("room") == "bathroom":
                recommendations.append({
                    "type": "health",
                    "priority": "high",
                    "title": "ตรวจสอบสุขภาพระบบทางเดินอาหาร",
                    "description": "การใช้เวลาในห้องน้ำนานอาจบ่งชี้ปัญหาสุขภาพ"
                })
                ...
                recommendations.append({
                    ...
                    "description": "พบการเคลื่อนไหวในช่วงกลางคืนบ่อย อาจต้องปรึกษาแพทย์"
                })
