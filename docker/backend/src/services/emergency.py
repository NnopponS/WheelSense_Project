...
class EmergencyService:
    ...
    EMERGENCY_TYPES = {
        ...
        "prolonged_stay": {
            "name_en": "Prolonged Stay",
            "severity": "medium",
            "auto_notify": False
        },
        "unusual_behavior": {
            "name_en": "Unusual Behavior",
            "severity": "medium",
            "auto_notify": False
        },
        "sos": {
    ...
    async def create_alert(
        ...
        event = await self.db.create_emergency(
            ...
            event_type=event_type,
            severity=severity,
            message=message or type_config.get("name_en", event_type),
            user_id=user_id
        )
            ...
            if status.get("user_in_room"):
                ...
                if stay_duration > threshold:
                    await self.create_alert(
                        ...
                        severity="medium",
                        message=f"User has been in {self._get_room_name_en(room)} for {stay_duration:.0f} minutes"
                    )
