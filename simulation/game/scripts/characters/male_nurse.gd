...
func _ready():
	...
	wander_timer.wait_time = randf_range(2.0, 7.0)
...
func _on_go_to_room(character_name: String, room_name: String):
	if character_name != "Male_nurse":
		return
	
	print("Male_nurse: Received command to go to ", room_name)
	...
	if room_position != Vector2.ZERO:
		...
		print("Male_nurse: Navigating to ", room_name, " at ", room_position)
	else:
		print("Male_nurse: Could not find room ", room_name)
