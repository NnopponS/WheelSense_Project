...
- **Workflow messaging (staff inboxes)**: Admin (`/admin/messages`), head nurse (`/head-nurse/messages`), and supervisor (`/supervisor/messages`) use the same **`GET/POST /api/workflow/messages`** contract for inbox, compose, and mark-read. In-app notification deep links for unread workflow messages use **`staffMessagesPath(role)`** in `frontend/lib/notificationRoutes.ts` (supervisor → `/supervisor/messages`, not the Operations Console).
...
| Supervisor | `/supervisor/workflow`, `/supervisor/monitoring`, `/supervisor/messages` | assigned patients, directives, tasks, schedules, audit, monitoring; workflow messages (inbox/compose) | tasks, schedules | directive acknowledge, workflow claim/handoff, task status, schedule status; read/reply workflow messages | explicit caregiver-patient access through linked caregiver profile; directive creation remains admin/head-nurse only |
