// Shared utilities for WheelSense monorepo.
// Provides enums, helpers to translate codes, and route helpers.
...
export const ewma = (previous: number | undefined, nextValue: number, alpha: number): number => {
  const prev = Number.isFinite(previous) ? Number(previous) : nextValue;
  return prev + alpha * (nextValue - prev);
};
