import mqtt from "mqtt";
import { z } from "zod";
import { Prisma, PrismaClient, WheelStatus } from "@prisma/client";
import { Server as SocketServer } from "socket.io";
import {
  directionText,
  motionText,
  statusText,
  distanceToUint16,
  RouteSnapshot,
  isSameRoute
} from "@wheelsense/shared";
import { logger } from "./logger";
import { AppConfig } from "./config";
import { KpiTracker } from "./kpi";
import { TelemetryForwarder } from "./redis";
import { TelemetryPayload } from "./types";

const payloadSchema = z.object({
  room: z.number().int(),
  room_name: z.string().optional(),
  wheel: z.number().int(),
  wheel_name: z.string().optional(),
  distance: z.number(),
  status: z.number().int(),
  motion: z.number().int(),
  direction: z.number().int(),
  rssi: z.number(),
  stale: z.boolean().optional(),
  ts: z.string(),
  gateway_ts: z.string().optional(),
  route_path: z.array(z.string()).optional(),
  route_latency_ms: z.number().optional(),
  route_recovery_ms: z.number().optional(),
  route_recovered: z.boolean().optional()
});

const toWheelStatus = (value: string | null | undefined): WheelStatus => {
  if (!value) return "UNKNOWN";
  const lower = value.toUpperCase() as WheelStatus;
  if ((Object.values(WheelStatus) as string[]).includes(lower)) {
    return lower;
  }
  return "UNKNOWN";
};

interface MqttDeps {
  prisma: PrismaClient;
  io: SocketServer;
  config: AppConfig;
  kpi: KpiTracker;
  forwarder: TelemetryForwarder;
}

export const createMqttIngest = ({ prisma, io, config, kpi, forwarder }: MqttDeps) => {
  const client = mqtt.connect(config.mqttUrl, { reconnectPeriod: 2000 });
  let lastRoutes = new Map<number, RouteSnapshot>();

  client.on("connect", () => {
    logger.info({ url: config.mqttUrl }, "MQTT connected");
    kpi.setMqttStatus("connected");
    client.subscribe(config.mqttTopic, (err) => {
      if (err) {
        logger.error(err, "Failed to subscribe MQTT topic");
      }
    });
  });

  client.on("close", () => {
    kpi.setMqttStatus("disconnected");
  });

  client.on("reconnect", () => kpi.setMqttStatus("reconnecting"));

  client.on("message", async (_topic, buffer) => {
    kpi.trackPacket();
    const rawText = buffer.toString();
    try {
      const raw = JSON.parse(rawText) as TelemetryPayload;
      const data = payloadSchema.parse(raw);

      const ts = new Date(data.ts);
      if (Number.isNaN(ts.getTime())) throw new Error("Invalid ts");
      const gatewayTs = data.gateway_ts ? new Date(data.gateway_ts) : null;

      const statusLabel = statusText(data.status);
      const motionLabel = motionText(data.motion);
      const directionLabel = directionText(data.direction);
      const wheelStatus = toWheelStatus(statusLabel);
      const distanceRounded = Math.round(data.distance * 100) / 100;
      const distanceRaw = distanceToUint16(data.distance);

      const result = await prisma.$transaction(async (tx) => {
        const room = await tx.room.upsert({
          where: { id: data.room },
          update: data.room_name ? { name: data.room_name } : {},
          create: {
            id: data.room,
            name: data.room_name ?? `Room ${data.room}`,
            rect_x: 0,
            rect_y: 0,
            rect_w: 100,
            rect_h: 100
          }
        });

        const wheel = await tx.wheel.upsert({
          where: { id: data.wheel },
          update: data.wheel_name ? { name: data.wheel_name } : {},
          create: {
            id: data.wheel,
            name: data.wheel_name ?? `Wheel ${data.wheel}`
          }
        });

        const rawData = await tx.rawData.create({
          data: {
            ts,
            gateway_ts: gatewayTs,
            room_id: data.room,
            wheel_id: data.wheel,
            direction_code: data.direction,
            direction_text: directionLabel,
            status_code: data.status,
            status_text: wheelStatus,
            motion_code: data.motion,
            motion_text: motionLabel,
            distance_m: new Prisma.Decimal(distanceRounded),
            distance_raw_uint16: distanceRaw,
            rssi: data.rssi,
            stale: data.stale ?? false,
            rx_ok: !(data.stale ?? false),
            route_path: data.route_path ?? [],
            route_latency_ms: data.route_latency_ms ?? null,
            route_recovery_ms: data.route_recovery_ms ?? null,
            route_recovered: data.route_recovered ?? null
          }
        });

        const presence = await tx.presence.upsert({
          where: { wheel_id: data.wheel },
          update: {
            online: !(data.stale ?? false),
            last_seen: ts,
            avg_rssi: data.rssi,
            rx_total: { increment: 1 },
            rx_ok: { increment: data.stale ? 0 : 1 },

            status_text: wheelStatus,
            motion_text: motionLabel,
            direction_text: directionLabel,
            distance_m: new Prisma.Decimal(distanceRounded),
            room_id: data.room
          },
          create: {
            wheel_id: data.wheel,
            online: true,
            last_seen: ts,
            avg_rssi: data.rssi,
            rx_total: 1n,
            rx_ok: data.stale ? 0n : 1n,
            rx_ratio: data.stale ? 0 : 1,
            status_text: wheelStatus,
            motion_text: motionLabel,
            direction_text: directionLabel,
            distance_m: new Prisma.Decimal(distanceRounded),
            room_id: data.room
          }
        });

        let routeSnapshot: RouteSnapshot | undefined;
        if (data.route_path && data.route_path.length > 0) {
          const snapshot = await tx.meshRouteSnapshot.create({
            data: {
              wheel_id: data.wheel,
              room_id: data.room,
              path: data.route_path,
              hop_count: data.route_path.length,
              recovered: data.route_recovered ?? false,
              recovery_ms: data.route_recovery_ms ?? null,
              latency_ms: data.route_latency_ms ?? null,
              observed_at: ts
            },
            include: {
              wheel: true
            }
          });

          routeSnapshot = {
            wheelId: snapshot.wheel_id,
            wheelName: snapshot.wheel.name,
            roomId: snapshot.room_id,
            path: snapshot.path,
            recovered: snapshot.recovered,
            recoveryMs: snapshot.recovery_ms ?? undefined,
            latencyMs: snapshot.latency_ms ?? undefined,
            observedAt: snapshot.observed_at.toISOString()
          };
        }

        return { room, wheel, rawData, presence, routeSnapshot };
      });

      kpi.registerWheel(data.wheel);
      kpi.trackPresence(data.wheel, !(data.stale ?? false));
      if (data.route_recovery_ms) {
        kpi.trackRecovery({
          wheelId: data.wheel,
          recoveryMs: data.route_recovery_ms,
          observedAt: Date.now()
        });
      }

      const telemetry = {
        wheel_id: data.wheel,
        wheel_name: result.wheel.name,
        room_id: data.room,
        room_name: result.room.name,
        status_code: data.status,
        status_text: statusLabel,
        motion_code: data.motion,
        motion_text: motionLabel,
        direction_code: data.direction,
        direction_text: directionLabel,
        distance_m: distanceRounded,
        distance_raw_uint16: distanceRaw,
        rssi: data.rssi,
        stale: data.stale ?? false,
        ts: ts.toISOString(),
        route_path: data.route_path ?? [],
        route_latency_ms: data.route_latency_ms ?? null,
        route_recovery_ms: data.route_recovery_ms ?? null,
        route_recovered: data.route_recovered ?? null
      };

      io.of("/rt").emit("telemetry", telemetry);
      if (result.routeSnapshot) {
        const previous = lastRoutes.get(result.routeSnapshot.wheelId);
        if (!isSameRoute(previous, result.routeSnapshot)) {
          io.of("/rt").emit("route", result.routeSnapshot);
          lastRoutes.set(result.routeSnapshot.wheelId, result.routeSnapshot);
        }
      }

      await forwarder.publishTelemetry(telemetry);
    } catch (err) {
      logger.warn({ err, rawText }, "Failed to process MQTT payload");
    }
  });

  return client;
};
