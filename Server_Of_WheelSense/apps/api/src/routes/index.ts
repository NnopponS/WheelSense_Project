...
export const createApiRouter = ({ prisma, kpi }: ApiDeps) => {
  ...
  router.get("/wheels", async (_req, res) => {
  ...
  router.get("/routes/live", async (_req, res) => {
    ...
    latest.forEach((row) => {
      if (row._max.observed_at) key.set(row.wheel_id, row._max.observed_at);
    });

    const snapshots = await prisma.meshRouteSnapshot.findMany({
      where: {
        OR: Array.from(key.entries()).map(([wheelId, observed]) => ({
          wheel_id: wheelId,
  ...
  router.get("/routes/history", async (req, res) => {
    ...
    if (params.wheel_id) where.wheel_id = Number(params.wheel_id);

    const limit = params.limit ? Math.min(Number(params.limit), 500) : 100;

    const data = await prisma.meshRouteSnapshot.findMany({
      where,
      include: {
        wheel: true,
        room: true
