...
# WheelSense product feature defaults. Only the Non-Secure and CM55 projects
# add these values to their compiler command lines.
WS_FEATURE_WIFI?=1
WS_FEATURE_BLE?=1
WS_FEATURE_CAMERA?=1
WS_FEATURE_ENVIRONMENT?=1
WS_FEATURE_MICROPHONE?=0
WS_FEATURE_SPEAKER?=0
WS_FEATURE_TOUCH?=1
...
ifneq ($(WS_FEATURE_BITSTREAM),0)
$(error WS_FEATURE_BITSTREAM must remain 0)
endif
ifneq ($(WS_FEATURE_SENSOR_STUDIO),0)
$(error WS_FEATURE_SENSOR_STUDIO must remain 0)
endif
ifneq ($(WS_FEATURE_DIGITAL_TWIN),0)
$(error WS_FEATURE_DIGITAL_TWIN must remain 0)
endif

WS_FEATURE_DEFINES=WS_FEATURE_WIFI=$(WS_FEATURE_WIFI) \
                   WS_FEATURE_BLE=$(WS_FEATURE_BLE) \
                   WS_FEATURE_CAMERA=$(WS_FEATURE_CAMERA) \
                   WS_FEATURE_ENVIRONMENT=$(WS_FEATURE_ENVIRONMENT) \
                   WS_FEATURE_MICROPHONE=$(WS_FEATURE_MICROPHONE) \
                   WS_FEATURE_SPEAKER=$(WS_FEATURE_SPEAKER) \
                   WS_FEATURE_TOUCH=$(WS_FEATURE_TOUCH) \
                   WS_FEATURE_MOTION_AI=$(WS_FEATURE_MOTION_AI) \
                   WS_FEATURE_HOST_SIM=$(WS_FEATURE_HOST_SIM) \
                   WS_FEATURE_BITSTREAM=0 \
                   WS_FEATURE_SENSOR_STUDIO=0 \
                   WS_FEATURE_DIGITAL_TWIN=0
...
DEFINES+=WS_TARGET_MTB_IPC=1
# Sensors on kit — gated by WheelSense feature flags
DEFINES+=IM_ENABLE_BOARD_CONFIG
ifeq ($(WS_FEATURE_MICROPHONE),1)
DEFINES+=IM_ENABLE_PDM_PCM
endif
ifeq ($(WS_FEATURE_TOUCH),1)
