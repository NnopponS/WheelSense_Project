...
/* WheelSense IPC shared queue — same physical SRAM as CM33 NS.
 * Both cores define this in .cy_shared_socmem which maps to the
 * m33_m55_shared region (0x262FC000). Both definitions land at the
 * same physical address. CM33 NS initializes; CM55 attaches. */
...
int main(void)
{
    ...
    /* Initialize WheelSense IPC shared queue (receiver side).
     * The shared queue was initialized by CM33 NS in shared SRAM.
     * CM55 attaches to the same shared memory instance. */
    (void)ws_ipc_transport_receiver_init(&ws_ipc_shared);
