...
class MCPServer:
    ...
    def _get_base_schedule_item(self, time: str, activity: str = None) -> dict:
        ...
        """
        base_schedule = self.state_manager._user_info["schedule"]
    ...
    def schedule_modifier(self, modify_type: str, time: str = None, 
                         ...
                         old_activity: str = None, user_message: str = None) -> dict:
        """
        Tool: Modify today's schedule using daily clone.
        
        This tool updates ONLY the schedule_modifications structure (daily clone), never the original schedule.
        All modifications apply to today's schedule only.
        ...
        Note: action and location are automatically derived from activity by the system.
              The LLM cannot directly set these fields.
        ...
        # Determine what "today" is based on custom clock (if using custom clock)
        # Check daily_clone_date to see if we're using a custom date
        mods = self.state_manager._user_info["schedule_modifications"]
        clone_date = mods.get("daily_clone_date")
        if clone_date:
            # Using custom clock - use the clone_date as "today"
            today = clone_date
            print(f"[SCHEDULE MODIFIER] Using custom clock date as 'today': {today}")
        else:
            # Using real clock - use real datetime
            today = datetime.now().strftime("%Y-%m-%d")
            print(f"[SCHEDULE MODIFIER] Using real date as 'today': {today}")
        ...
        try:
            if modify_type == "add":
                ...
                # Build schedule item with time and activity (LLM only provides these)
                ...
                # Allow multiple items at the same time - no duplicate check
                ...
                # ALSO add to base schedule to make it recurring for all future days
                base_schedule = self.state_manager._user_info["schedule"]
                # Check if item already exists in base schedule
                ...
                if not exists_in_base:
                    ...
                    print(f"[SCHEDULE MODIFIER] Also added to base schedule for recurring: {activity} at {time}")
                else:
                    # Update existing item in base schedule
                    for idx, item in enumerate(base_schedule):
                        if item.get("time") == time:
                            base_schedule[idx] = new_item.copy()
                            base_schedule.sort(key=lambda x: x.get("time", ""))
                            print(f"[SCHEDULE MODIFIER] Updated base schedule item at {time}: {activity}")
                            break
                
                # Debug: Verify the add was applied with all fields
                ...
                return {
                    ...
                    "time": time,
                    "activity": activity,
                    "message": f"Added schedule item at {time}: '{activity}'",
                    "error": None
                }
