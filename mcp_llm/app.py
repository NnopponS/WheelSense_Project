...
# Apply any pending device updates from previous run (e.g., from LLM actions)
# This must happen before widgets are instantiated to ensure they show the correct state
if "_pending_device_updates" in st.session_state and st.session_state._pending_device_updates:
    updates = st.session_state._pending_device_updates
    print(f"[UI SYNC] Applying {len(updates)} pending device updates to session state")
    for key, new_val in updates.items():
        st.session_state[key] = new_val
        print(f"[UI SYNC] Updated {key} -> {new_val}")
    # Clear updates after applying
    del st.session_state["_pending_device_updates"]
        ...
        with st.expander("➕ Add Schedule Item", expanded=False):
            ...
            if st.button("Add Schedule Item", key="add_schedule_btn", type="primary"):
                if new_schedule_time and new_schedule_activity:
                    ...
                    if result.get("success"):
                        # Clear form
                        st.session_state.new_schedule_time = ""
                        st.session_state.new_schedule_activity = ""
                        st.session_state.new_schedule_location = "None"
                        st.session_state.new_schedule_devices = []
                ...
                for i, tool_call in enumerate(tool_calls):
                    ...
                    if tool_result.get("success"):
                        if tool_result.get("tool") == "e_device_control":
                            ...
                            # Queue update for the UI toggle state (applied on next run)
                            toggle_key = f"toggle_{room}_{device}"
                            new_val = (action.upper() == "ON")
                            
                            if "_pending_device_updates" not in st.session_state:
                                st.session_state._pending_device_updates = {}
                            st.session_state._pending_device_updates[toggle_key] = new_val
