...
| 2D — production web UI | IN PROGRESS (P2D.1-P2D.6 COMPLETE; role rename head_caregiver/caregiver applied; Docker browser regression pending) | Phase 2C complete | `phase-2d-production-ui-report.md`; queue-first workbench; emergency triage with acknowledge/resolve; task creation with explicit ownership; patient detail canonical at patients/[id]; messages verified; tsc/build/jest PASS; supervisor→head_caregiver, observer→caregiver rename committed e287d4f9a |
...
| 3 — sensors/orientation | IN PROGRESS (P3.1–P3.3 host-only GREEN; P3.4–P3.7 gated by Gate B) | Gate A for host-only; Gate B for target integration | See P3.1–P3.3 TDD evidence below; host_sim 8/8 tests PASS; target build unaffected (new files not in target SOURCES list) |
...
P3.1–P3.3 are software-only. No board was programmed, no sensor was read, and no hardware PASS is claimed. The new source files (`ws_environment.c`, `ws_imu_orientation.c`) are NOT in the target `SOURCES` list and no target file includes the new headers, so the target build is completely unaffected. The SHT40 and DPS368 conversion formulas are from the respective datasheets; the orientation classifier uses a dominant-axis gravity approach with dwell/hysteresis/stability/rate-limit gates per phase-3-brief.md §3.10. Hardware validation of actual sensor reads, axis remap correctness, and physical orientation transitions remains deferred to Gate B (P3.4) and Gate D (P3.7). The BMI270 is used ONLY for touchscreen orientation — it is NOT wheelchair-motion input and does NOT run a motion classifier (`WS_FEATURE_MOTION_AI=0`).
