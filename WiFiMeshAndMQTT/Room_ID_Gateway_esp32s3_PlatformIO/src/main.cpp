...
void receivedCallback(uint32_t from, String &msg) {
  ...
  RouteState &state = gRouteStates[from];
  unsigned long nowMs = millis();
  bool routeRecovered = false;
  uint32_t routeRecoveryMs = 0;
  if (state.pathKey.length() > 0 && pathKey != state.pathKey) {
    routeRecovered = true;
    if (state.lastSeenMs > 0 && nowMs >= state.lastSeenMs) {
      routeRecoveryMs = nowMs - state.lastSeenMs;
    }
  }
