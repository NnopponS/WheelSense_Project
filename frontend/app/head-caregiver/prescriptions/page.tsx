...
export default function HeadCaregiverPrescriptionsPage() {
  const { t } = useTranslation();
  const queryClient = useQueryClient();

  const prescriptionsQuery = useQuery({
    queryKey: ["head_caregiver", "prescriptions", "list"],
    queryFn: () => api.listPrescriptions(),
  });

  const patientsQuery = useQuery({
    queryKey: ["head_caregiver", "prescriptions", "patients"],
    queryFn: () => api.listPatients({ limit: 300 }),
  });

  const specialistsQuery = useQuery({
    queryKey: ["head_caregiver", "prescriptions", "specialists"],
  ...
  const createPrescriptionMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["head_caregiver", "prescriptions", "list"] });
    },
  });
