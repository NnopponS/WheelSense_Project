...
import { z } from "zod";
import { CalendarClock, ClipboardList, Plus, UserCog, Users, Briefcase } from "lucide-react";
...
function roleLabel(role: string, t: (key: TranslationKey) => string): string {
  ...
  if (role === "head_nurse") return t("shell.roleHeadNurse");
  if (role === "supervisor") return t("shell.roleSupervisor");
  if (role === "observer") return t("shell.roleObserver");
  ...
  return (
    ...
    >
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("shell.roleHeadNurse")}</p>
            <p className="text-xl font-semibold">{staffByRole.head_nurse}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("shell.roleSupervisor")}</p>
            <p className="text-xl font-semibold">{staffByRole.supervisor}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t("shell.roleObserver")}</p>
