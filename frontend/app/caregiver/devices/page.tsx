...
export default function CaregiverDevicesPage() {
  const { t } = useTranslation();
  const nowMs = useFixedNowMs();

  const devicesQuery = useQuery({
    queryKey: ["caregiver", "devices", "list"],
    queryFn: async () => {
  ...
  const predictionsQuery = useQuery({
    queryKey: ["caregiver", "devices", "predictions"],
    queryFn: async () => {
  ...
  const alertsQuery = useQuery({
    queryKey: ["caregiver", "devices", "alerts"],
    queryFn: () => api.listAlerts({ status: "active", limit: 240 }),
  });

  const vitalsQuery = useQuery({
    queryKey: ["caregiver", "devices", "vitals"],
