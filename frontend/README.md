...
- **Workflow care tasks — calendar vs board (2026-04)**:
  - **Operations Console** (`components/workflow/OperationsConsole.tsx`, routes such as `/head-nurse/workflow`, `/supervisor/workflow`, `/observer/workflow`) remains the **multi-domain** surface (queue of tasks + schedules + directives, transfers, audit, reports). **No Kanban** there — board views stay on task-focused pages to avoid mixing incompatible item types. When the console is embedded under **`/head-nurse/tasks`**, **`/observer/tasks`**, or **`/supervisor/tasks`** (tab **Workflow**), inner panel selection uses **`?wtab=`** (`queue` \| `transfer` \| `coordination` \| `audit` \| `reports`) so it does not overwrite the hub’s **`?tab=`**. **`GET /api/analytics/wards/summary`** is not available to **observer**; the console skips that fetch for observers and uses fallbacks. Standalone `/workflow` may still use legacy **`?tab=`** for those panels; the console reads **`wtab` first**, then falls back.
  ...
  - **Tasks hubs (shared shell)**: **`/head-nurse/tasks`**, **`/observer/tasks`**, and **`/supervisor/tasks`** share **Tasks** (**`WorkflowTasksHubContent`**, variants `head-nurse` \| `observer` \| `supervisor`) and **Workflow** (`OperationsConsole`). **Checklist** / **Timeline** tabs align across roles: head nurse **Checklist** is workspace oversight (`HeadNurseShiftChecklistsPage`); observer and supervisor **Checklist** is personal **`ShiftChecklistMePanel`** (`GET /api/shift-checklist/me`). **Timeline** embeds **`WardTimelineEmbed`** (same calendar/agenda UI as **`/head-nurse/timeline`**, separate React Query cache keys per role). **Alerts** are not embedded in **`/observer/tasks`** or **`/supervisor/tasks`** — use **`/observer/alerts`** / **`/supervisor/emergency`** (or role inbox) from the sidebar. Observer adds **List** (`ObserverTaskListPanel.tsx`) and refresh / 403 handling. **New tasks** use the console **Transfer** panel (`?wtab=transfer` on standalone `/workflow` routes). **`/supervisor/workflow`** remains a legacy hub (workflow \| calendar \| directives); sidebar **Workflow** points to **`/supervisor/tasks`**. i18n: `workflowTasks.kanban.*`, `workflowTasks.hubBoardTitle` / `hubBoardSubtitle`, `headNurse.tasksHub.*`, `observer.tasks.*`, `wardTimeline.*`.
...
- `app/head-nurse/` - ward operations and staffing; **dashboard** (`/head-nurse`) embeds floorplan + on-duty roster + priority alerts/tasks + **staff context strip** (see “Workflow care tasks” / “Head nurse dashboard” bullets above); **tasks hub** (`/head-nurse/tasks`) uses **`WorkflowTasksHubContent`** (calendar, Kanban, same filters/stats as observer `/observer/tasks`)
...
- `app/supervisor/` - command center, **`/supervisor/tasks`** hub (Tasks \| Workflow \| Checklist \| Timeline; staff scheduling stays on **`/supervisor/calendar`**), legacy **`/supervisor/workflow`**, directives redirect, emergency map, prescriptions, **workflow messages** (`/supervisor/messages` — inbox/sent/compose via `api.listWorkflowMessages` / `sendWorkflowMessage` / `markWorkflowMessageRead`, same APIs as `/head-nurse/messages`)
- `app/observer/` - monitoring, **`/observer/tasks`** hub (Tasks \| Workflow \| Checklist \| Timeline; task **Calendar \| Board** toggles live on the **Tasks** tab; alerts at **`/observer/alerts`**), standalone **`/observer/workflow`**, staff **`/observer/calendar`**, patients
